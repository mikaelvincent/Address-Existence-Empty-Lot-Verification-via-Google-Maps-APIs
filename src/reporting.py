...
Inputs:
- data/enhanced.csv (from Sprint 6)
- Optional: data/review_log_completed.csv (from Sprint 7)
...
## Method notes
- Final labels are produced by the deterministic rule set (Sprints 1–6) with **optional human‑review overrides** (Sprint 7).
- This report was generated by **Sprint 8** consolidation tooling. No new API calls are made here.
...
def run_reporting(
    ...
    """Main entry: merge human reviews (if any), write final CSV + report + JSONL.
...
def main() -> None:
    p = argparse.ArgumentParser(description="Sprint 8 — Consolidation, QA, and final package.")
    p.add_argument("--enhanced", required=True, help="Path to data/enhanced.csv")
    p.add_argument(
        "--reviews",
        required=False,
        default=None,
        help="Optional path to data/review_log_completed.csv (from Sprint 7).",
    )
    p.add_argument(
        "--final-out",
        required=False,

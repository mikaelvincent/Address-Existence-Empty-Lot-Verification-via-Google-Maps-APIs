...
def _aggregate_metrics(rows: List[Dict[str, str]], overrides: List[OverrideInfo]) -> RunMetrics:
    ...
    # New (Track A — input correctness)
    input_incorrect_true = sum(1 for r in rows if (r.get("input_incorrect_flag") or "").strip().lower() == "true")
    equivalence_counts = collections.Counter((r.get("input_equivalence") or "").strip() for r in rows)
    issue_cnt: collections.Counter[str] = collections.Counter()
    for r in rows:
        s = (r.get("input_issue_codes") or "").strip()
        if not s:
            continue
        for tok in s.split("|"):
            tok = tok.strip()
    ...
    return RunMetrics(
        ...
        input_incorrect_true=input_incorrect_true,
        equivalence_counts=dict(sorted(equivalence_counts.items(), key=lambda kv: kv[0])),
        top_issue_codes=dict(sorted(issue_cnt.items(), key=lambda kv: (-kv[1], kv[0]))),
    )
...
## Input correctness (Track A)
- **Rows with input_incorrect_flag==true:** {metrics.input_incorrect_true}

### Equivalence distribution
{_format_kv_table(metrics.equivalence_counts, "Input equivalence", "Rows")}

### Top input issue codes
{_format_kv_table(metrics.top_issue_codes, "Issue code", "Occurrences")}
...
- Track A (input correctness) is derived from Google Address Validation + Geocoding signals (Place ID / proximity / per-component changes).
- This report is generated by consolidation tooling; **no new API calls** are made here.
- Caching & compliance: only **lat/lng** (TTL ≤ {cfg.cache_policy.latlng_ttl_days} days) and permitted **Google IDs** (e.g., Place IDs) are cached; no scraping or bulk export of Google content.
"""
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        for r in rows:
            ...
            rec = {
                ...
                # Input correctness
                "input_incorrect_flag": r.get("input_incorrect_flag", ""),
                "input_equivalence": r.get("input_equivalence", ""),
                "input_issue_codes": (r.get("input_issue_codes") or "").split("|") if (r.get("input_issue_codes") or "") else [],
            }
